...
class GenerativeAudioEngine:
    ...
    def _generate_chunk(self) -> np.ndarray:
        ...
        for i, f in enumerate(freqs):
            fm = np.sin(2.0 * np.pi * (f * 0.25) * t_seq) * (0.8 + 0.4 * np.sin(self.time_val * 0.3 + i))
            phase_inc = 2.0 * np.pi * f * t_seq + fm
            amp = 0.08 + 0.06 * np.sin(self.time_val * 0.5 + i * 1.6)
            out += np.sin(phase_inc + self.phase[i]) * amp * np.exp(-((t_seq % 0.5) * 4.0))
            self.phase[i] = (self.phase[i] + 2.0 * np.pi * f * dt) % (2.0 * np.pi)

        kick_env = max(0, np.sin(self.time_val * np.pi * 2.0 * 1.5)) ** 24.0
        kick = np.sin(2.0 * np.pi * 45.0 * t_seq - kick_env * 15.0) * kick_env * 0.4
        out += kick

        scratch_env = max(0, np.sin(self.time_val * np.pi * 3.0)) ** 8.0
        out += np.random.normal(0, 0.02 * scratch_env, CHUNK_SIZE)
...
float map(in vec3 p, out float matID, out vec4 stateOut) {
    ...
    float waveDisturb = texture(audioWave, fract(p.x * 0.05 + time * 0.1)).r * audioBass * 0.6;
    ...
    float inkRipples = texture(audioWave, fract(p.x * 0.1 + p.z * 0.05 - time * 0.3)).r * audioTreble * 0.15;
...
void main() {
    ...
    vec3 rd = ca * normalize(vec3(p.xy, 1.8 - audioBass * 0.1));
...
def build_hold_frame(lines: Sequence[str], size: Sequence[int]) -> np.ndarray:
    ...
    frame[..., 0] = np.clip(244.0 - 40.0 * (1.0 - yg) - stripe * 12.0, 0, 255).astype(np.uint8)
    frame[..., 1] = np.clip(238.0 - 20.0 * (1.0 - yg) - stripe * 8.0, 0, 255).astype(np.uint8)
    frame[..., 2] = np.clip(225.0 - 16.0 * (1.0 - yg) - stripe * 7.0, 0, 255).astype(np.uint8)
    ...
    for i, line in enumerate(lines):
        color = (200, 50, 50) if i == 0 else (100, 110, 120) if i >= len(lines) - 2 else (35, 30, 28)
...
def build_hud_frame(left: Sequence[str], right: Sequence[str], size: Sequence[int], hud_scale: float) -> np.ndarray:
    ...
    for x, y, pw, ph, lines in [(lx, ly, l_w, l_h, left), (rx, ry, r_w, r_h, right)]:
        _fill_rect(frame, x, y, x + pw, y + ph, (244, 238, 225, 172))
        _fill_rect(frame, x, y, x + scale, y + ph, (24, 75, 165, 210))
        cy = y + pad_y
        for i, line in enumerate(lines):
            s = t_scale if x == lx and i == 0 else scale
            col = (35, 30, 28, 245) if x == lx and i == 0 else (200, 50, 50, 238) if "OFF" in line else (50, 60, 70, 230)
...
class GarageHeatShow(mglw.WindowConfig):
    title = "Living Sketchbook - Volumetric Ink Audio Reactive"
    gl_version = (4, 5)
    ...
    def _hud_lines(self) -> Sequence[Sequence[str]]:
        ...
        return (
            ["LIVING SKETCHBOOK LAB", "3D VOLUMETRIC INK", f"{w}X{h} MAP {max(1, int(np.ceil(w/ts)))}X{max(1, int(np.ceil(h/ts)))}", f"SIM STEP {self.args.substeps} RAYMARCH {self.args.ray_steps}", f"FX {self.args.fx_intensity:.1f} CAM {self.args.camera_speed:.1f}", f"CPU WORKERS {self.args.cpu_workers}"],
    ...
    def add_arguments(cls, parser) -> None:  # type: ignore[override]
        ...
        parser.add_argument("--ray-steps", type=int, default=110, help="Maximum raymarch steps per pixel")
